...
async function enviarPedidoVendedor(pedido, items) {
  const rows = items.map(it =>
    `<tr>
      <td style="padding:6px 12px;border-bottom:1px solid #eee">${it.nombre_producto}</td>
  ...
  <div style="font-family:Inter,sans-serif;max-width:640px;margin:0 auto">
    ...
    <div style="background:#f9fafb;padding:24px 32px;border-radius:0 0 12px 12px;border:1px solid #e5e7eb">
      ...
      <p style="color:#9ca3af;font-size:0.8rem;margin-top:20px">Pedido #${pedido.id} — ${new Date().toLocaleString('es-MX')}</p>
    </div>
  </div>`;
...
module.exports = { enviarPedidoVendedor, enviarResetPassword };
